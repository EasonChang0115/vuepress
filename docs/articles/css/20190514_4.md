...
tags:
  - javascript
description: html5出來後，a標籤可以在href屬性中寫入hash tag(dom物件ID)，讓畫面跳轉到該物件上。但是卻無法很平順的跳轉，畫面是閃過去直接更換到該dom的位址，使用者體驗比較差。
...
meta:
  ...
  - name: og:description
    content: html5出來後，a標籤可以在href屬性中寫入hash tag(dom物件ID)，讓畫面跳轉到該物件上。但是卻無法很平順的跳轉，畫面是閃過去直接更換到該dom的位址，使用者體驗比較差。
  - name: og:image
    content: /css.png
---
