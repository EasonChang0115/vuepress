---
title: javascript 物件與函數(二)
category: javascript
tags:
  - javascript
description: 這邊會討論到有關call by value 和 call by reference 的概念(所有的primitive types都是by value)。以及函數參數的寫法，加入ES6之後會有什麼改變。。
image: https://images.unsplash.com/photo-1547371026-a3952b80acb5?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=701&q=80
meta:
  - name: og:url
...
### By Value 傳值
---
若是primitive value的位置，他會用by value的方式
...
2.當b被設定等於a，但這個變數b不會得到新的記憶體位置，而是指向a的記憶體位置。(沒有新的物件被創造，沒有複製與新增)
...
console.log(a.name) // sherry 即使是function的參數也是by reference
...
## Array: collection of objects 混和物件的陣列
...
1. javascript可以呼叫函數並不傳入參數。參數會像變數一樣，在執行環境被創造時，會hositing。
...
// no pass lang
```
3. 關鍵字arguments：javascript engine會把傳入的參數，設定到關鍵字arguments中。

* 不管有沒有傳入參數，arguments都會被創造出來。
* function即使沒有設定傳入參數對應的變數，參數都會依序丟到arguments中。
* arguments像是陣列，但它不是陣列，他只有陣列一部份的功能。但大部分的功能都可以用陣列來處理。
...
可以檢查arguments長度是否等於 0，可以用陣列表示法取值
