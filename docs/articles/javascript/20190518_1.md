...
# 克服javascript 3 物件與函數(三)

## Function overloading 函數重載
在其他高階語言中，函數可以依傳入參數的不同數量、型態來使用同一個參數名稱但卻指向不同函數。但是在javascript中，函數即是物件，所以沒有這個處裡函數的功能。
但因為js的function為一等公民，可以有很多種選擇。
* 可用函數的方式來呼叫同一支函數。(預設情況)
```js
