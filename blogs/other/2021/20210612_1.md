...
meta:
  - name: og:url
    content: https://easonchang0115.github.io/blogs/other/2021/20210612_1.html
...
假設，使用物件的程式碼同時還要負責實體化該物件，程式碼將會變得非常複雜。他必須了解到很多事情，包含建立那些物件、需要那些建構函數、建構後如何使用物件甚至是管理物件。這會降低程式碼的內聚性，大大的提供維護的難度。所以過早的實體化，會導致過分專注於物件的其他行為。
...
這樣我們可以開店了，在麥當勞這家餐廳中，我們不需要關心薯條、麥脆雞如何實作，只要提供的物件給使用者，使用者會知道如何去使用被麥當勞提供的產品，這樣的模式稱為簡單工廠(Simple Factory)。
...
簡單工廠模式讓我們把食物的創建跟使用分開來了，如果要改變處理方式，則要去修改那個麥當勞或是給我一個新的工廠就可以了。我只要關注在我的食物是麥當勞的食物就可以了
...
這樣寫起來會越來越多，感覺越來越不好維護。所以我們把相關的產品組成一個產品族，交給同一個工廠來生產。每當寫一個產品就會就會多了非常多的物件類別。
非常不好管理，加上如果我要吃雞腿，還要浪費記憶體空間去宣告雞腿的店，實在是不好維護跟使用。所以我們定義產生相關物件的介面來針對子類實作所有的產品，這就是抽象工廠模式。
