...
生活上常見的 Adapter 模式的例子好比是電源轉接頭等，透過這個方法，可以把原本不相容的類別套至系統上或是需要的類別中。
...
講簡單點就是我們可以透過定義一個較為抽象的方法，讓子類別<span style="color: red;">繼承</span>，子類別再透過各自的行為完善該方法的實作。
...
該活的需求方突然要加入新的動物 - 貓，要跟隔壁老王的貓一樣可以跳來跳去。當然，身為研發者就必須去看看老王的貓是怎麼跳的，發現其實老王是利用第三方開源工具來實現貓跳的動作：
...
於是乎，咱們因為時間的關係，沒辦法自己實作貓跳的動作，所以我們也用了這個第三方套件。但發現：
...
透過使用 Adapter 模式，我們可以繼續使用 Animal。Adapter 模式最常見的用途就是要讓介面保持多型。
...
5. [Facade Pattern – Design Patterns (ep 9)](https://www.youtube.com/watch?v=2PKQtcJjYvc&list=PLrhzvIcii6GNjpARdnO4ueTUAVR9eMBpc&index=8&ab_channel=ChristopherOkhravi)