...
description: 圖形是組成一個畫面的最基本要素。舉凡常見的矩形、圓形甚至三角形，只要透過旋轉、縮放、變形到重複繪圖等等動作，都可以用獨一無二的畫面作品呈現出來。
這次就來介紹幾個常用的幾何圖形的 API 以及注意事項吧。
...
meta:
  ...
  - name: og:description
    content: 圖形是組成一個畫面的最基本要素。舉凡常見的矩形、圓形甚至三角形，只要透過旋轉、縮放、變形到重複繪圖等等動作，都可以用獨一無二的畫面作品呈現出來。
這次就來介紹幾個常用的幾何圖形的 API 以及注意事項吧。
  - name: og:image
